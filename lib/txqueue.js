...
class TxQueue {
    ...
    enqueue(argObj){
        ...
        if(this._queue.length >= this._maxQueueLength){
            ...
            top.abort(new QueueError(`queue too long: ${this._queue.length}, discarded top of queue. Was doing ${this._current?("queued "+new Date(this._current.queued)):"nothing"}`))
        }
