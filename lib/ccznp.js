...
CcZnp.prototype._sendSREQ = function (argObj, callback) {
    ...
    this.once(srspEvt, function (result) {
        ...
        if (result === '__timeout__') {
            logSrsp('<-- %s:%s, TIMEOUT, %s', argObj.subsys, argObj.cmd, valObjFormat(payload));
            callback(new Error('request timeout'));
        } else if (result === '__error__') {
            logSrsp('<-- %s:%s, ERROR, %s', argObj.subsys, argObj.cmd, valObjFormat(payload));
...
CcZnp.prototype._mtIncomingDataHdlr = function (err, data) {
    ...
    if (data.type === 'SRSP') {
        logSrsp('<-- %s:%s, %o', subsys, cmd, valObjFormat(result));
