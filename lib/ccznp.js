...
class CcZnp extends EventEmitter {
    ...
    async start(){
        ...
        this._unpi = new Unpi({ lenBytes: 1, phy: sp });

        this._unpi.on('data', data=>this._parseMtIncomingData(data));
        this._unpi.on('error', err=>this._handleMtError(err));
    }
    ...
    async request (subsys, cmd, valObj) {
        assert(this._unpi)
        const argObj = new ZpiObject(subsys, cmd, valObj)
        return await this._pipeline.execute(argObj)
    }

    sendCmd (type, subsys, cmdId, payload) {
        assert(this._unpi)
        // for debugging only, may disrupt communicator
