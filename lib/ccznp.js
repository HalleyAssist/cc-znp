...
class CcZnp extends EventEmitter {
    ...
    async start(startPromise = null, drain = true){
        this._init = Q.defer()
        ...
        do {
            ...
            try {
                ...
                if(values.includes(rsp.value)){
                    const e = this._communicator.takePending({cmd: "echo", subsys: this._mainSubsys})
                    if(e) e.deferred.reject('__sync__')
                    continue
                }
            } catch(ex){
                //not empty
    ...
    async _parseMtIncomingData(data){
        ...
        try {    
            ...
            if(data.fcs !== data.csum){
                debug(`Error while receiving ${data.subsys}:${data.cmd} (len:${data.len}), invalid checksum`)
            } else {
                debug(`Error while receiving ${data.subsys}:${data.cmd} (len:${data.len}), error: ${ex}`)
            }
            return false
        }
        try {    
            ...
            assert(typeof data.payload === 'object', 'data.payload should be an object after parsing');
        } catch(ex){
            debug(`Error while parsing ${argObj} (len:${data.len}), error: ${ex}`)
            this._communicator.clear(data)
            return false
        }
        ...
        try {
            ...
            debug(`Error while receiving ${argObj} (len:${data.len}), error: ${ex}`)
            return false
        }
    }
    ...
    async _closeOpen(){
        ...
        if(this._communicator){
            ...
            await this._communicator.clearAll()
        }
    }
...
zmeta.getSubsystems().forEach(function (subsys) {
    ...
    CcZnp.prototype[reqMethod] = function (cmdId, valObj, logging = true) {
        return this.request(subsys, cmdId, valObj, logging);
    };
});
