...
for(var i in zmeta.Commands){
    MT[i] = zmeta.Commands[i]
}
...
var innerListeners = {
    ...
    spErr: function (err) {
        ...
        if(this._sp.isOpen && !this._sp._errClosing) {
            this._sp._errClosing = true
            this._close()
        }
    },
    spClose: function () {
    ...
    handleMtError: function (result) {
        this._handleMtError(result);
    }
};
...
CcZnp.prototype.init = function (spCfg, callback) {
    ...
    var sp
    if(this._sp instanceof SerialPort){
        sp = this._sp
    }else if(spCfg.sp){
        this._sp = sp = spCfg.sp
    }else{
        sp = this._sp = new SerialPort(spCfg.path, spCfg.options)
    }

    var unpi = this._unpi = (this._unpi instanceof Unpi) ? this._unpi : new Unpi({ lenBytes: 1, phy: sp });

    // Listeners for inner use
    var parseMtIncomingData = innerListeners.parseMtIncomingData.bind(this),
        spOpenLsn = innerListeners.spOpen.bind(this),
        spErrLsn = innerListeners.spErr.bind(this),
        spCloseLsn = innerListeners.spClose.bind(this),
        handleMtError = innerListeners.handleMtError.bind(this)
    ...
    unpi.removeAllListeners('data');
    unpi.removeAllListeners('error');
...
function request(self, subsys, cmd, valObj, callback, queued){
    ...
    if (argObj.type !== 'SREQ' && argObj.type != 'AREQ'){
        return
    }

    // prepare for transmission with spinlock
    if (self._spinLock) {
        log[argObj.type](' [queued] %s:%s, %o', argObj.subsys, argObj.cmd, valObjFormat(valObj));
        var entry = {
            subsys: subsys,
            cmd: cmd,
            fn: function () {
                try {
                    request(self, subsys, cmd, valObj, callback, true);
                }catch(ex){
        ...
        if(self._txQueue.length >= self._maxQueueLength){
            var top = self._txQueue.shift()
            top.abort("queue too long")
        }
        self._txQueue.push(entry);
        return
    }
    self._spinLock = true;

    var logFromQueue = queued ? " (FROM QUEUE)":""
    if (argObj.type === 'SREQ') {
        log[argObj.type]('--> %s:%s, %o%s', argObj.subsys, argObj.cmd, valObjFormat(valObj), logFromQueue);
        self._sendSREQ(argObj, valObj, callback);
    } else if (argObj.type === 'AREQ') {
        log[argObj.type]('--> %s:%s, %o%s', argObj.subsys, argObj.cmd, valObjFormat(valObj), logFromQueue);
        self._sendAREQ(argObj, callback);
    }
}

CcZnp.prototype.request = function (subsys, cmd, valObj, callback) {
    request(this, subsys, cmd, valObj, callback)
};
...
CcZnp.prototype._sendSREQ = function (argObj, valObj, callback) {
    ...
    this.once(srspEvt, function (result) {
        ...
        if (result === '__timeout__') {
            log.SRSP('<-- %s:%s, TIMEOUT, %o', argObj.subsys, argObj.cmd, valObjFormat(valObj));
            callback(new Error('request timeout'));
        } else if (result === '__error__') {
            log.SRSP('<-- %s:%s, ERROR, %o', argObj.subsys, argObj.cmd, valObjFormat(valObj));
...
CcZnp.prototype._handleMtError = function(error){
    this._mtIncomingDataHdlr(error, null);
}
...
CcZnp.prototype._mtIncomingDataHdlr = function (err, data) {
    ...
    if (err) {
        // just print out. do nothing if incoming data is invalid
        debug(err); 
        return;
    }
    ...
    if (data.type === 'SRSP') {
        ...
        log.SRSP('<-- %s:%s, %o%s', subsys, cmd, valObjFormat(result), listeners ? "":" (NO LISTENERS)");
        if (listeners)
            this.emit(rxEvt, result);
    } else if (data.type === 'AREQ') {
        log.AREQ('<-- %s:%s, %o', subsys, cmd, valObjFormat(result));
        rxEvt = 'AREQ';
        msg = {
            subsys: subsys,
            ind: cmd,
...
CcZnp.prototype._cleanup = function(){
    this._txQueue = null;
    this._txQueue = [];
    this._sp = null;
    this._unpi = null;
