...
class ZpiObject {
    ...
    async parse(type, zBuf) {
        ...
        if (!results.length) throw new Error(`Expected a single parsed message got ${results.length} from data ${zBuf.toString('hex')}`)
        return results[0]
    }
        ...
        for (let i = 0; i < this.args.length; i++) {
            ...
            switch (type) {
                ...
                case 'listbuffer': {  // [ 0x0001, 0x0002, 0x0003, ... ]
                    let tempBuf = Buffer.alloc(val.length * 2)
