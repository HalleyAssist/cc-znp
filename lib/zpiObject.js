...
class ZpiObject {
            ...
            if (Array.isArray(args)) {
                ...
                reqParams.forEach(function (arg, idx) {
                    if (!args.hasOwnProperty(arg.name))
                        throw new Error(`The argument object at-least one incorrect property ${arg.name}`);
    ...
    async parse (type, zBuf) {
        ...
        if (rspParams) {                        // [ { name, type }, ... ]
            ...
            throw new Error('Response parameter definitions not found.');
        }
        ...
        const results = parser.process(zBuf)
