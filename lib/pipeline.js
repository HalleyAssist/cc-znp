...
class Pipeline {
    ...
    async _send(argObj, logging = true){
    ...
    async __execute(cancellationState, argObj, logging = true){
        ...
        const sender = ()=>Q.fcall(()=>this._send(argObj, logging))

        try {
            const execution = sender()
            
            try {
                await cancellationState.promiseWrap(execution.timeout(2200))
            } catch(ex){
                ...
                try {
                    ...
                    ret = await cancellationState.promiseWrap(Q.cancelledRace([execution.timeout(8800), reAlignedSend]))
                }catch(ex){
                    if(ex.code != 'ETIMEDOUT') throw ex
                    this._communicator._logTimeout(argObj)
    ...
    _execute(cancellationState, argObj, logging = true){
        const ret = this.__execute(cancellationState, argObj, logging)
