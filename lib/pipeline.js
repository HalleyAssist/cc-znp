...
class Pipeline {
    ...
    async _realign(canceled){
        ...
        while(!canceled.status && ++i < 10){
            ...
            let e 
            try {
                e = await Q.timeout(this._execute(existing?existing.argObj:argObj), true, existing ? existing.defer : null, 701)
            } catch(ex){
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    ret = await Q.timeout(Q.cancelledRace([this._abort.promise, execution, reAlignedSend]), 8800) //5s total
                }catch(ex){
                    if(ex.code != 'ETIMEDOUT') throw ex
                    this._communicator._logTimeout(argObj)
            ...
            if(ret === DefaultValue) ret = await Promise.race([this._abort.promise, execution])
        }catch (ex) {
            try {
