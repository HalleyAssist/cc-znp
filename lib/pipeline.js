...
class Pipeline {
    ...
    async _realign(canceled){
        ...
        while(!canceled.status && ++i < 10){
            ...
            let e 
            try {
                e = await Q(this._execute(existing?existing.argObj:argObj)).timeout(701)
            } catch(ex){
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    await Q(Q.cancelledRace([execution, reAlignedSend])).timeout(8800) //5s total
                }catch(ex){
                    if(ex.code != 'ETIMEDOUT') throw ex
                    this._communicator._logTimeout(argObj)
            ...
            ret = await Promise.race([this._abort.promise, execution])
        }catch (ex) {
