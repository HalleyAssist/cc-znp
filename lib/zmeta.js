...
function LightEnum(cmds, fn){
    this._forwards = cmds
    this._backwards = {}
    var extractorFn = fn || function(a){return a}
    this._extractorFn = extractorFn
    for(var i in cmds){
        this._backwards[this._extractorFn(cmds[i])] = i
    }
}
...
for(var i in zpiMeta){
    zmeta.Commands[i] = new LightEnum(zpiMeta[i], function(a){return a.cmdId})
}
