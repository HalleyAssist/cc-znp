    ...
    */ 
    ...
    addPending(argObj, deferred, logging = true){
        for(const p of this._pending){
            if(p.argObj == argObj) throw new Error("argObj already pending")
            if(argObj.subsys == p.argObj.subsys && argObj.cmd == p.argObj.cmd) throw new Error("subsys & cmd already pending")
        }
        this._pending.push({
            argObj,
            deferred,
    ...
    async receive(data){
        ...
        if (data.type === 'SRSP') {
            ...
            if(pending.logging){
                Logging.request.SRSP('%s:%s %s <=> %s%s', data.subsys, data.cmd, Logging.valObjFormat(pending.argObj.valObj), Logging.valObjFormat(data.payload, true), checksumValid?"":" (INVALID CHECKSUM)");
            }
            ...
            Logging.request.AREQ('<== %s:%s %s%s', data.subsys, data.cmd, Logging.valObjFormat(data.payload, true), checksumValid?"":" (INVALID CHECKSUM)");
        }
