...
for(const k in Constants.SYS.resetReason){
    resetReasonLookup[Constants.SYS.resetReason[k]] = k
}

const nwkStatusLookup = {}
for(const k in Constants.Common.nwkStatus){
    nwkStatusLookup[Constants.Common.nwkStatus[k]] = k
}
    ...
    if(subsys === "AF"){
        ...
        if(process.env.NODE_ENV !== 'production'){
            if(cmd === 'dataConfirm' && valObj.trans){
                ...
                if(afReq && afReq.id == valObj.trans){
                    ret = ret?ret:Object.assign({}, valObj)
        ...
        } else if(cmd === 'networkStatus'){
            ret = ret?ret:Object.assign({}, valObj)
            ret.code = new enumLookup(valObj.code, nwkStatusLookup)
        }
    } else if(subsys === 'SYS'){
        ...
        } else if(cmd === 'resetInd' && isRsp) {
            ret = ret?ret:Object.assign({}, valObj)
            ret.reason32 = new enumLookup(valObj.reason32, resetReasonLookup)
        }
    }
